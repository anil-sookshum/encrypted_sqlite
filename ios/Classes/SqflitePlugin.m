...
NSString *const _paramPassword = @"password";
...
+ (NSArray *)toSqlArguments:(NSArray *)rawArguments {
    ...
    if (![SqflitePlugin arrayIsEmpy:rawArguments]) {
        for (NSUInteger i = 0; i < [rawArguments count]; i++) {
            [array addObject:[SqflitePlugin toSqlValue:rawArguments[i]]];
        }
    }
    return array;
}
...
+ (NSDictionary *)fromSqlDictionary:(NSDictionary *)sqlDictionary {
    ...
    for (NSString *key in sqlDictionary.keyEnumerator) {
        NSObject *sqlValue = sqlDictionary[key];
        dictionary[key] = [SqflitePlugin fromSqlValue:sqlValue];
    }
    return dictionary;
}
